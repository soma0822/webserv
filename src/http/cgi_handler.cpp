...
Option<HTTPResponse *> CGIHandler::Handle(const IConfig &config, HTTPRequest *cgi_req, RequestContext req_ctx) {
  if (cgi_req->GetHeaders().count("LOCATION") == 1 && cgi_req->GetHeaders().at("LOCATION").at(0) == '/') {
    if (req_ctx.count >= 10)
      return (Some(HTTPResponse::Builder().SetStatusCode(http::kInternalServerError).Build()));
    req_ctx.count++;
