...
// ゲッター
bool LocationContext::get_can_auto_index() const { return can_auto_index_; }
int LocationContext::get_limit_client_body() const {
  return limit_client_body_;
}
const std::string& LocationContext::get_return() const { return return_; }
const std::string& LocationContext::get_alias() const { return alias_; }
const std::string& LocationContext::get_root() const { return root_; }
...
const std::map<std::string, std::string>& LocationContext::get_error_page()
    const {
  return error_page_;
}
// セッター
...
void LocationContext::set_return(const std::string& ret) { return_ = ret; }
void LocationContext::set_alias(const std::string& alias) { alias_ = alias; }
void LocationContext::set_root(const std::string& root) { root_ = root; }
...
void LocationContext::add_error_page(const std::string& key,
...
std::ostream& operator<<(std::ostream& os, LocationContext& obj) {
  os << "index: ";
  for (std::vector<std::string>::const_iterator it = obj.get_index().begin();
       it != obj.get_index().end(); it++) {
    os << *it << " ";
  }
  os << "\nerror page: ";
  ...
  os << "\ncgiPath: ";
  for (std::vector<std::string>::const_iterator it = obj.get_cgi_path().begin();
       it != obj.get_cgi_path().end(); it++) {
    os << *it << " ";
  }
  os << "\ncgiExtention: ";
  for (std::vector<std::string>::const_iterator it =
           obj.get_cgi_extention().begin();
       it != obj.get_cgi_extention().end(); it++) {
    os << *it << " ";
  }
  os << "\nautoindex: " << (obj.get_can_auto_index() == true ? "on" : "off");
  os << "\nreturn: "
     << (!obj.get_return().empty() ? obj.get_return() : "no set");
