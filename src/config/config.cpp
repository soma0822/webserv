...
const ServerContext &Config::SerchServer(const std::string &port, const std::string &server_name){
  long ret = -1;
  for (unsigned int i = 0; i < server_.size(); i++){
    if (server_.at(i).HavePort(port) && (server_.at(i).HaveServerName(server_name) || ret == -1))
      ret = i;
  }
