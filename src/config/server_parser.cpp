...
// パーサー
bool ServerParser::parse_error_page(const std::vector<std::string> &value,
                                    ServerContext &server) {
...
bool ServerParser::parse_index(const std::vector<std::string> &value,
                               ServerContext &server) {
  if (value.size() == 0) return false;
  server.add_index(value[0]);
  return true;
}
bool ServerParser::parse_ip(const std::vector<std::string> &value,
                            ServerContext &server) {
  if (value.size() != 1) return false;
  server.set_ip(value[0]);
  return true;
}
bool ServerParser::parse_root(const std::vector<std::string> &value,
                              ServerContext &server) {
  if (value.size() != 1) return false;
  server.set_root(value[0]);
  return true;
}
bool ServerParser::parse_server_name(const std::vector<std::string> &value,
                                     ServerContext &server) {
  if (value.size() == 0) return false;
  for (std::vector<std::string>::const_iterator it = value.begin();
       it != value.end(); it++)
...
bool ServerParser::parse_port(const std::vector<std::string> &value,
                              ServerContext &server) {
